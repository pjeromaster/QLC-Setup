...
function write_color {
    ...
    color=$2
    case $color in
        "Red") echo -e "\x1B[31m$text\x1B[0m";;
        "Green") echo -e "\x1B[32m$text\x1B[0m";;
        "Yellow") echo -e "\x1B[33m$text\x1B[0m";;
        "Cyan") echo -e "\x1B[36m$text\x1B[0m";;
        *) echo "$text";;
    esac
}
...
function install_theme {
    ...
    sed -i '' -e "s|{{resources_path}}|${resourcesPath}|g" "$stylePath" || { write_color "Failed to update the resource path in the style file." "Red"; exit 1; }
